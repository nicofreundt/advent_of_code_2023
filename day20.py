...
if __name__ == "__main__":
    ...
    with open(FILE) as file:
        ...
        active_flops = []
        conjunction_input_defaults = {con: {k: 0 for k, _ in filter(lambda x: con in x[1], typed_modules.items())} for con in conjunctions}
        ...
        for _ in range(BUTTON_CLICKS):
            ...
            while cur_signals:
                ...
                next_modules = typed_modules[cur_module]
                ...
                if cur_module in conjunctions:
                    conjunction_input_defaults[cur_module][last_module] = pulse
                    
                    if pulse and len(conjunction_input_defaults[cur_module]) == 1:
                        next_pulse = 0
                    elif all(conjunction_input_defaults[cur_module].values()):
                        next_pulse = 0
                    else:
                        next_pulse = 1
    ...
    with open(FILE) as file:
        ...
        low_rx_state = conjunction_input_defaults[rx_predecessor].copy()
        
        low_rx = False
        button_clicks = 0
        
        while not low_rx:
            if all(low_rx_state.values()):
            ...
            while cur_signals:
                ...
                if last_module in conjunction_input_defaults[rx_predecessor] and pulse:
